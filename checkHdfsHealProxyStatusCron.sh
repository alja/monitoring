...
echo "==================================================================="
na=`grep Attach hdfs.log  |wc -l`
echo "Attach  ($na)x"
 grep Atta hdfs.log | tail -1 | perl -ne 'if(/\[(\d+)-(\d+)-(\d+) (.*) (\-\d+)\]\[Info(.*)Attach\(\)(.*)root\?/){print "[$4] $7root\n";}'

echo -e "\n"

nd=`grep Detach hdfs.log  |wc -l`
echo "Detach ($nd)x"
 grep Deta hdfs.log | tail -1 | perl -ne 'if(/\[(\d+)-(\d+)-(\d+) (.*) (\-\d+)\]\[Info(.*)Detach\(\)(.*)root\?/){print "[$4]  $7root\n";}'
